...
def findPrefixPath(basePat,treeNode): 								#参数basePat表示输入的频繁项，treeNode为当前FP树中对应的第一个节点（可在函数外部通过headerTable[basePat][1]获取）
	...
	while treeNode != None:											#利用头指针表，#通过链表遍历相似元素项,调用ascendTree获取相似元素项的前缀路径
		...
		if len(prefixPath) > 1: 									#注意,这里使用了数组过滤,把叶子节点过滤掉，得出前缀路径，放入字典中
			condPats[frozenset(prefixPath[1:])] = treeNode.count 	#prefixPath[1:]  从第二个元素开始到最后一个元素  treeNode.count:该元素在该路径上出现次数
...
# # print(initSet)
myFPtree,myheaderTab = createTree(initSet,3)
myFPtree.disp() 													#输出的时候顺序有可能变化，不完全按照路径顺序
# # print(myheaderTab)
...
# print(rPrefixPath)
# sPrefixPath = findPrefixPath('s',myheaderTab['s'][1])
# print(sPrefixPath)
# tPrefixPath = findPrefixPath('t',myheaderTab['t'][1])
# print(tPrefixPath)
